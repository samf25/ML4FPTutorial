...
class PixelClusterDataset(Dataset):
    """Dataset for pixel cluster HDF5 files from create_hdf5_pixelhits.py.

    Each sample is a cluster represented as a zero-padded sequence of raw
    pixel hits with features (energy, time, x, y, z), sorted by energy
    descending, plus a binary label (1 = signal, 0 = BIB).
    """

    def __init__(self, file_path, num_evt=-1, max_hits=9, label_override=None):
        self.file_path = file_path
        ...
        # Bulk-read all vlen arrays in one HDF5 call each (5 calls total
        # instead of 5*N), then fill the tensor in a Python loop.
        print(f"  Pre-loading {self._n} samples from {os.path.basename(file_path)}...")
        rh = self._file['raw_hits']
        all_e = rh['energy'][:self._n]
        ...
        for i in range(self._n):
            ...
            if nhits > 0:
                ...
                self._X[i, :nhits, 0] = torch.from_numpy(e[order].astype(np.float32))
    ...
    def cluster_features(self, keys=None):
        """Return dict of cluster-level feature arrays {name: (N,) array}."""
